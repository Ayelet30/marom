  export const environment = {
    production: false,
    firebase: {
      apiKey: "AIzaSyCiGz6RP0PuGkPK5W31KEqWzPx7aQuq-tM",
      authDomain: "marom-9f242.firebaseapp.com", // אמור להיות app.com לא .storage.app
      projectId: "marom-9f242", // ה-id של הפרויקט ב-Firebase
      storageBucket: "marom-9f242.firebasestorage.app", // ה-storage bucket שלך
      messagingSenderId: "361323286558",
      appId: "1:361323286558:web:C9F9DCAA439A38E06DDE0F",
      measurementId: "G-MER9G4FCDP" // אם יש לך את המזהה למדידה, הוסף אותו
    },
    
    hashavshevet: {
      apiUrl: '/api',
      token: '854826E559D793F0EE35958B74CA6A17',
      stationId: 'a8361de2-9f06-4c35-9dd7-45d292eacad1',
      company: 'demo',
      username: '1'
    },

    apiUrl: 'http://localhost:3500'

  };

  